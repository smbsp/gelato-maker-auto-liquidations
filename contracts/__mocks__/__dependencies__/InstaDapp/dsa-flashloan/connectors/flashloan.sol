...
contract DydxFlashHelpers is Helpers {
    ...
    function calculateTotalFeeAmt(DydxFlashInterface dydxContract, uint256 amt)
        internal
        view
        returns (uint256 totalAmt)
    {
        uint256 fee = dydxContract.fee();
        if (fee == 0) {
            totalAmt = amt;
        } else {
            uint256 feeAmt = wmul(amt, fee);
            totalAmt = add(amt, feeAmt);
        }
    }

    function calculateFlashFeeAmt(
        DydxFlashInterface dydxContract,
        uint256 flashAmt,
        uint256 amt
    ) internal view returns (uint256 totalAmt) {
        uint256 fee = dydxContract.fee();
        if (fee == 0) {
            totalAmt = amt;
        } else {
            uint256 feeAmt = wmul(flashAmt, fee);
            totalAmt = add(amt, feeAmt);
        }
    }
}
...
contract LiquidityAccessHelper is DydxFlashHelpers {
    ...
    function addFeeAmount(
        ...
        DydxFlashInterface dydxContract =
            DydxFlashInterface(getDydxFlashAddr());

        uint256 totalFee = calculateFlashFeeAmt(dydxContract, flashAmt, _amt);
...
contract LiquidityAccess is LiquidityAccessHelper {
    ...
    function flashBorrowAndCast(
        ...
        DydxFlashInterface(getDydxFlashAddr()).initiateFlashLoan(
            tokens,
            amts,
            route,
            data
    ...
    function flashPayback(
        ...
        uint256 totalFeeAmt = calculateTotalFeeAmt(dydxContract, _amt);

        _transfer(
            payable(address(getDydxFlashAddr())),
            tokenContract,
            totalFeeAmt
        );
        ...
        address[] memory tokens = new address[](1);
        uint256[] memory amts = new uint256[](1);
        uint256[] memory totalFeeAmts = new uint256[](1);
        tokens[0] = token;
        amts[0] = amt;
...
contract LiquidityAccessMulti is LiquidityAccess {
    ...
    function flashMultiPayback(
        address[] calldata tokens,
        uint256[] calldata amts,
        uint256[] calldata getId,
        uint256[] calldata setId
    ) external payable {
        uint256 _length = tokens.length;
        DydxFlashInterface dydxContract =
            DydxFlashInterface(getDydxFlashAddr());

        uint256[] memory totalAmtFees = new uint256[](_length);
        for (uint256 i = 0; i < _length; i++) {
            uint256 _amt = getUint(getId[i], amts[i]);
